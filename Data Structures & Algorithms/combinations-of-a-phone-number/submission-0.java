class Solution {
    ...
    private void backtrack(int startDigit, String digits, StringBuilder current, List<String> result) {
        if (current.length() == digits.length()) {
            result.add(current.toString());
            return;
        }

        for (int i = startDigit; i < digits.length(); i++) {
            ...
            for (int j = 0; j < curLetters.length(); j++) {
                ...
                backtrack(i + 1, digits, current, result);
